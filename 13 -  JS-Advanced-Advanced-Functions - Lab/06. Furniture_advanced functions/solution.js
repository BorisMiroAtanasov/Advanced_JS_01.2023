function solve() {
  ...
  function buy() {
    const selectItems = items.filter(i => i.isChecked()).map(e => e.item);
    const names = selectItems.map(i => i.name).join(', ');
    const total = selectItems.reduce((a,c) => a + Number(c.price), 0);
    const deckFactor  = selectItems.reduce((a,c,i,arr) => a + Number(c.decFactor)/arr.length, 0);

    output.textContent = [
      `Bought furniture: ${names}`,
      `Total price: ${total.toFixed(2)}`,
      `Average decoration factor: ${deckFactor}`
    ].join('\n');
  }
